...
⟨ λ HPQR, HPQR.elim 
  ( λ HP, ⟨inl HP, inl HP⟩ ) $ λ ⟨HQ, HR⟩, ⟨inr HQ, inr HR⟩, 
  λ ⟨HPQ, HPR⟩, HPQ.elim ( λ HP, inl HP ) 
  $ λ HQ, HPR.elim ( λ HP, inl HP ) $ λ HR, inr ⟨HQ, HR⟩ ⟩
...
axiom ad_ass (a b c : B) : a + b + c = a + (b + c)
axiom mu_ass (a b c : B) : a * b * c = a * (b * c)
axiom ad_com (a b : B) : a + b = b + a
axiom mu_com (a b : B) : a * b = b * a
axiom ma_dis (a b c : B) : a * (b + c) = a * b + a * c
axiom mu_one (a : B) : a * one = a
axiom ad_zer (a : B) : a + zer = a
axiom ad_inv (a : B) : ∃! b : B, a + b = one
axiom mu_inv (a : B) : a ≠ zer → ∃! b : B, a * b = zer

noncomputable def neg (a : B) : B := classical.some (ad_inv a)
noncomputable def inv (a : B) (ha : a ≠ zer) : B 
:= classical.some (mu_inv a ha)

notation -a := neg a
notation a⁻¹ := inv a

theorem mu_zer (a : B) : a * zer = zer :=
begin
...
if x = 0 then 0 else real.exp ((real.log x) / n)
lemma nth_root_def {x : ℝ} (hx : x ≠ 0) {n : ℕ} : nth_root x n = real.exp (real.log x / n) :=
dif_neg hx
theorem nth_root_unique {x y : ℝ} {n : ℕ}
(Hxpos : 0 < x) (Hypos : 0 < y) (Hnpos : 0 < n) (Hynx : y ^ n = x) : y = nth_root x n := sorry



open complex

theorem abs_pow (x : ℂ) : ∀ n : ℕ, abs(x ^ n) = (abs x) ^ n
| 0 := by simp
| (nat.succ n) := by rw [pow_succ, pow_succ, complex.abs_mul, abs_pow]

theorem complex_nth_root_abs (zn : ℂ) (z : ℂ) (n : ℕ) (hn : n > 0) : z ^ n = zn → abs z = nth_root (abs zn) n :=
begin
  intro hzn,()
  cases classical.em(zn = 0),
  { simp [h, complex.abs_zero, nth_root],
    apply @pow_eq_zero _ _ _ n, rwa h at hzn },
  { have hzn' : (abs z) ^ n = abs zn := by rw [←abs_pow, hzn],
    apply nth_root_unique (abs_pos.mpr h) (abs_pos.mpr _) hn hzn',
    by_contra,
    rw not_not at a, rw [a, zero_pow hn, eq_comm] at hzn, apply h hzn }
end

theorem complex_nth_root_arg (zn : ℂ) (z : ℂ) (n : ℕ) (hn : n > 0) 
: z ^ n = zn → ∃ k : ℤ, arg z = arg(zn) / n + 2 * real.pi * k / n :=
begin
  intro hzn,
...
lemma lm2 (x : int) : (x - 1)*(x - 2) = x ^ 2 - 3 * x + 2 :=    
calc
...
begin 
  ...
  show (x - 1) * (x - 2) = 0, by
    calc
      (x - 1) * (x - 2) = x^2 - 3*x + 2 : by rw lm2
                    ... = 0 : by rw HX

end

  ---rw mul_self_iff_eq_one
/-import data.real.basic

inductive equals (a : ℕ) : ℕ → Prop | refals : equals a
theorem equals_rfl (a : ℕ) : equals a a := equals.refals _
theorem equals_symm (a b : ℕ) : equals a b → equals b a := λ ab, by { cases ab, exact ab }
theorem equals_trans (a b c : ℕ ) : equals a b → equals b c → equals a c := λ ab bc, by { cases ab, cases bc, exact ab }

def R : Type := {x : ℝ // x^2 < 0}
theorem good : ¬ ∃ t : R, true 
:= λ a, Exists.dcases_on a 
    (λ t ht, not_lt_of_ge 
      (pow_two_nonneg (t.val)) 
      (t.property))

theorem very_bad (r : R) : false := good (exists.intro r trivial)

theorem secret_to_universe_and_stuff (x : ℕ) 
: ((∀ (n : ℕ), x ∣ n ^ 7 - n) ∧ ∀ (x' : ℕ), (∀ (n : ℕ), x' ∣ n ^ 7 - n) → x' ∣ x) ↔ x = 42 :=
begin
  split,
  { intro hHCF, cases hHCF with hCF hH },
  { }
end
-/
