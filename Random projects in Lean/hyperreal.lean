...
def is_deriv_at (f : ℝ → ℝ) (x y : ℝ) := 
∀ h : ℝ*, infinitesimal h → h ≠ 0 → is_st (((lift f) (x + h) - (lift f) (x)) / h) y

-- MUST CHANGE: ALL infinitesimals h, not just epsilon
/-- The hyperreal derivative is the standard part of the slope of an infinitesimal secant -/
--def is_deriv_at (f : ℝ → ℝ) (x f'x : ℝ) := is_st (((lift f) (x + ε) - (lift f) (x)) / ε) f'x

noncomputable def deriv_at (f : ℝ → ℝ) (x : ℝ) : ℝ :=
if H : ∃ f'x : ℝ, is_deriv_at f x f'x then classical.some H else 0

lemma is_deriv_at_id : ∀ x, is_deriv_at id x 1 :=
λ x h Hhi Hh0, by rw [lift_id, id.def, id.def, add_sub_cancel', div_self Hh0]; 
exact is_st_refl_real _





lemma deriv_at_eq_st (f : ℝ → ℝ) (x : ℝ) : 
  ∀ h : ℝ*, infinitesimal h → h ≠ 0 → deriv_at f x = st (((lift f) (x + h) - (lift f) (x)) / h) :=
begin
unfold deriv_at st, split_ifs with hs,
intros h hi h0, split_ifs with hd, 
end

lemma is_deriv_at_iff_is_st_zero (f : ℝ → ℝ) (x f'x : ℝ) : 
  is_deriv_at f x f'x ↔ is_st (((lift f) (x + ε) - (lift f) (x) - f'x * ε) / ε) 0 :=
⟨ λ hS, by rw [sub_div, mul_div_cancel _ epsilon_ne_zero, ←sub_self f'x];
  exact is_st_sub hS (is_st_refl_real _),
    λ hS, by unfold is_deriv_at;
  rw [←zero_add f'x, ←sub_add_cancel (_ / ε) f'x, ←mul_div_cancel (↑f'x) epsilon_ne_zero, ←sub_div, 
    mul_div_cancel _ epsilon_ne_zero]; exact is_st_add hS (is_st_refl_real _) ⟩

def is_deriv (f f' : ℝ → ℝ) := ∀ x : ℝ, is_deriv_at f x (f' x)

noncomputable def deriv (f : ℝ → ℝ) : ℝ → ℝ :=
if h : ∃ f' : ℝ → ℝ, is_deriv f f' then classical.some h else 0

theorem is_deriv_at_of_tendsto (f : ℝ → ℝ) (x f'x : ℝ) :
  tendsto (λ h, (f (x + h) - f (x) - f'x * h) / h) (nhds 0) (nhds 0) → is_deriv_at f x f'x :=
λ hd, (is_deriv_at_iff_is_st_zero _ _ _).mpr (is_st_of_tendsto (tendsto.comp tendsto_inverse_at_top_nhds_0_nat hd))

/-theorem tendsto_of_is_deriv_at (f : ℝ → ℝ) (x f'x : ℝ) :
is_deriv_at f x f'x → tendsto (λ h, (f (x + h) - f (x) - f'x * h) / h) (nhds 0) (nhds 0) := 
λ hs, begin 
rw is_deriv_at_iff_is_st_zero at hs, 
change is_st (of_seq (λ n, (f (x + n⁻¹) - f x - f'x * n⁻¹) / n⁻¹)) 0 at hs,
simp only [metric.tendsto_nhds_nhds, dist, sub_zero],
intros e he,
show ∃ (δ : ℝ) (H : δ > 0), ∀ {d : ℝ}, abs d < δ → abs ((f (x + d) - f x - f'x * d) / d) < e,
end
#check metric.tendsto_nhds_nhds
theorem deriv_at_transfer (f : ℝ → ℝ) (x f'x : ℝ) : 
  is_deriv_at f x f'x ↔ tendsto (λ h, (f (x + h) - f (x) - f'x * h) / h) (nhds 0) (nhds 0) := 
⟨ tendsto_of_is_deriv_at _ _ _, is_deriv_at_of_tendsto _ _ _ ⟩
-/
end analysis
